      ...
      <el-form class="w-full">
        <el-row :gutter="12">
          <el-col :xs="24" :sm="24" :md="5" :lg="5" :xl="5" class="mb-[1em]">
            <el-form-item>
              <el-select placeholder="Chọn Tỉnh/Thành phố" class="w-full" v-model="filters.cityCode" @change="onCityChange" ref="city" clearable>
                <el-option v-for="(city, i) in cities" :key="'city' + i" :value="city.code" :label="city.name_with_type">{{ city.name_with_type }}</el-option>
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :xs="24" :sm="24" :md="5" :lg="5" :xl="5" class="mb-[1em]">
            <el-form-item>
              <el-select placeholder="Chọn Quận/Huyện" class="w-full" v-model="filters.districtCode" :disabled="!filters.cityCode" ref="district" clearable>
                <el-option v-for="(item, i) in districts" :key="'district' + i" :value="item.code" :label="item.name_with_type">{{ item.name_with_type }}</el-option>
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :xs="24" :sm="24" :md="4" :lg="4" :xl="4" class="mb-[1em]">
            <el-form-item>
              <el-select placeholder="Chọn Sân" class="w-full" v-model="filters.typePlace">
                <el-option v-for="(item, i) in types" :key="'type' + i" :value="item.id" :label="item.name">{{ item.name }}</el-option>
              </el-select>
            </el-form-item>
          </el-col>
    ...
    <section v-for="(item, i) in places" :key="'type' + i">
      <div v-if="item.place.length">
        ...
        <el-row :gutter="24" class="mb-[2em]">
          <el-col :xs="12" :md="12" :sm="12" :lg="6" :xl="6" v-for="(place, i) in item.place" :key="'stadium' + i" class="mb-[1em]">
